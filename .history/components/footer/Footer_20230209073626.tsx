  ...
  const [isLoading, setLoading] = useState(true);
	return (
			<footer className="bg bg-blue-900 text-white pb-3 pt-4 font-extralight flex flex-col" id="a-propos-de-nous">
              ...
              <p className="flex flex-col py-4 items-center justify-center md:justify-start">
                {STORES.map((item: any, index: number) => (
                    <Link
                        href="#"
                        target="_blank"
                        key={`liens-${index}`}>
                      <a className="block items-center py-2 w-48 h-14 relative border border-red-400">
                      <Image
                          layout={'fill'}
                          src="/images/google-app-store-badge.svg"                          alt={item?.label}
                      />{item.image}
                      </a>
                    </Link>
                ))}
