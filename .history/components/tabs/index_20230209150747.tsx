    ...
    <section className='container px-3 md:px-0 py-10'>
      ...
      {items.map((item: any, index: number) => (
          <section key={`tab-${index}`} className={
              classNames(
                'grow border-b-4 py-2 px-10',
                { 'bg-white border-white block': activeTab === `tab-${index}` },
                { 'hidden': activeTab !== `tab-${index}` }
                )
              }>
             {item.infos.map((info: any, index: number) => (
              <article key={`info-${index}`} className={classNames('grow border-b-4 py-2')}>
                <h3 className='pb-10 font-bold text-2xl'>{info.label}</h3>
                <RenderHtmlContent content={info.text} classes="text-lg"/>
              </article>
            ))}
          </section>
        ))}
